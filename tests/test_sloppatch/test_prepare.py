...
import pytest
from sloppatch.prepare import RawHunkValidationError, RawPatchValidationError, raise_validate_raw_hunk, raise_validate_raw_patch
...
class TestRaiseValidateHunk:
    def test_valid_hunk(self) -> None:
        hunk = make_raw_hunk(1, 3, 3, 3, changes=[
    ...
    def test_before_length_mismatch(self) -> None:
        hunk = make_raw_hunk(1, 99, 3, 3, changes=[
            make_change(RawAct.Context, "a"),
        ])
        with pytest.raises(RawHunkValidationError, match="Original line count.+does not match"):
            raise_validate_raw_hunk(hunk)

    def test_after_length_mismatch(self) -> None:
        hunk = make_raw_hunk(1, 1, 3, 99, changes=[
            make_change(RawAct.Context, "a"),
        ])
        with pytest.raises(RawHunkValidationError, match="New line count.+does not match"):
            raise_validate_raw_hunk(hunk)
    ...
    def test_only_deletions_valid(self) -> None:
        ...
        hunk = make_raw_hunk(1, 2, 3, 0, changes=[
    ...
    def test_only_additions_valid(self) -> None:
        hunk = make_raw_hunk(1, 0, 3, 2, changes=[
...
class TestRaiseValidatePatch:
    ...
    def test_invalid_after_line_too_small(self) -> None:
        ...
        with pytest.raises(RawPatchValidationError, match="Original start line and new start line validation failed"):
            raise_validate_raw_patch([h1, h2])
